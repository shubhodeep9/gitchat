...
class FocusMixin(object):
    ...
    def mouse_event(self, size, event, button, x, y, focus):
        if focus and hasattr(self, '_got_focus') and self._got_focus:
            self._got_focus()
        return super(FocusMixin,
                     self).mouse_event(size, event,
                                       button, x, y, focus)
...
class Commander(urwid.Frame):
    ...
    """ Simple terminal UI with command input on bottom line
    and display frame above
    ...
    def output(self, line, style=None):
        if style and style in self._output_styles:
            line = (style, line)
        self.body.add(line)
        # since output could be called asynchronously form other threads we
        # need to refresh screen in these cases
